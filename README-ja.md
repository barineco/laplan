...
- 型名は Laplan 正式形 (`str`, `i32`, `f64` 等)。rule を宣言すれば solver が経路を解く。
...
- **emit**: synthesis の生成対象。この cratis のコードを出力する
